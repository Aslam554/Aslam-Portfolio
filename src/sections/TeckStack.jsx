      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {filtered.map((tech, index) => (
          <motion.div
            ...
            className="group rounded-2xl bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-white/10 p-5 shadow-md hover:shadow-xl backdrop-blur-lg hover:border-cyan-400 transition-all duration-300"
          >
            <div className="flex flex-col items-center justify-center">
              <div className="mb-3">{iconMap[tech.key]}</div>
              <p className="text-sm text-white font-semibold group-hover:text-cyan-300 transition">
                {tech.key}
              </p>
            </div>
          </motion.div>
